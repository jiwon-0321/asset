...
function findNoteById(notes = [], noteId = "") {
  return (Array.isArray(notes) ? notes : []).find((note) => String(note?.id || "").trim() === String(noteId || "").trim()) || null;
}
...
module.exports = async (request, response) => {
  ...
  try {
    ...
    if (request.method === "PUT") {
      ...
      if (!noteId) {
        sendJson(response, 400, { error: "수정할 메모 ID를 찾지 못했습니다." });
        return;
      }

      const existingNote = findNoteById(notes, noteId);
      if (!existingNote) {
        sendJson(response, 404, { error: "수정할 메모를 찾지 못했습니다." });
        return;
      }
    ...
    const noteId = String(payload.id || "").trim();
    if (!noteId) {
      sendJson(response, 400, { error: "삭제할 메모 ID를 찾지 못했습니다." });
      return;
    }

    if (!findNoteById(notes, noteId)) {
      sendJson(response, 404, { error: "삭제할 메모를 찾지 못했습니다." });
      return;
    }
    ...
    sendError(response, error, "메모 처리에 실패했습니다.", { statusResolver: resolveMutationErrorStatus });
  }
};
