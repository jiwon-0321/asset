...
const DEFAULT_GEMINI_MODEL_CANDIDATES = Object.freeze([DEFAULT_GEMINI_MODEL, "gemini-2.5-flash"]);
const DEFAULT_TIMEOUT_MS = 15000;
const DEFAULT_MAX_IMAGE_BYTES = 2 * 1024 * 1024;
const DEFAULT_MAX_TOTAL_IMAGE_BYTES = 2560 * 1024;
const DEFAULT_MAX_IMAGE_COUNT = 4;
const ENV_PATHS = [".env.local", ".env"];
const GEMINI_API_KEY_ENV_KEYS = Object.freeze(["GEMINI_API_KEY", "GOOGLE_API_KEY", "GOOGLE_GENAI_API_KEY"]);
...
function parseGeminiModelCandidates(rawValue = "") {
  return uniqueTrimmedStrings(String(rawValue || "").split(/[\n,]/u));
}

function resolveTradePhotoAssistConfig() {
  const configuredModel = resolveEnvValue(["TRADE_PHOTO_ASSIST_GEMINI_MODEL"]);
  const configuredModelCandidates = parseGeminiModelCandidates(resolveEnvValue(["TRADE_PHOTO_ASSIST_GEMINI_MODELS"]));
  const modelCandidates = uniqueTrimmedStrings([
    configuredModel,
  ...
  return {
    apiKey: resolveEnvValue(GEMINI_API_KEY_ENV_KEYS),
    model: modelCandidates[0] || DEFAULT_GEMINI_MODEL,
    modelCandidates,
...
async function extractTradeDraftWithVision(imagePayloads = [], options = {}) {
  const { apiKey, modelCandidates, timeoutMs } = resolveTradePhotoAssistConfig();
  if (!apiKey) {
    throw createTradePhotoAssistError(
      "Gemini API 키가 없어 사진 초안을 만들 수 없습니다. GEMINI_API_KEY, GOOGLE_API_KEY, GOOGLE_GENAI_API_KEY 중 하나를 설정해주세요.",
      503
    );
  }
  ...
  for (const model of modelCandidates) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const requestError = createTradePhotoAssistError(`사진 초안 요청에 실패했습니다. ${message}`, response.status || 502);
      ...
      if (error?.statusCode && isRetryableGeminiModelError(error) && model !== modelCandidates[modelCandidates.length - 1]) {
        lastError = error;
        continue;
      }
      if (error?.statusCode) {
  ...
  throw lastError || createTradePhotoAssistError("사진 초안을 만들지 못했습니다.", 502);
}
