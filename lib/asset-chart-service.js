...
function normalizePointLabel(value, locale = "ko-KR") {
...
async function fetchCryptoChart(symbol, fetchImpl = fetch) {
  const url = new URL("https://api.upbit.com/v1/candles/days");
  url.searchParams.set("market", symbol);
  url.searchParams.set("count", String(DEFAULT_POINT_COUNT));
...
async function fetchUsStockChart(rootDir, symbol, fetchImpl = fetch) {
  ...
  url.searchParams.set("outputsize", String(DEFAULT_POINT_COUNT));
...
async function buildAssetChartSnapshot({ rootDir, market, symbol, name, fetchImpl = fetch }) {
  ...
  const chart =
    normalizedMarket === "crypto"
      ? await fetchCryptoChart(normalizedSymbol, fetchImpl)
      : await fetchUsStockChart(rootDir, normalizedSymbol, fetchImpl);
