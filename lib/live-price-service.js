  ...
  if (usStockInstruments.length) {
    if (hasFreshMarketCache) {
      ...
      if (!apiKey) {
        ...
        try {
          fx = await fetchYahooFxRate(fetchImpl);
          const stockResults = await Promise.allSettled(
            usStockInstruments.map((instrument) => fetchYahooStockQuote(instrument, fetchImpl))
          );
          const marketQuotes = {};
          ...
          stockResults.forEach((result, index) => {
            ...
            if (result.status === "fulfilled") {
              ...
              errors.push(result.reason?.message || `${instrument.symbol} 현재가를 불러오지 못했습니다.`);
          ...
          livePriceCache.market = {
            ...
            fx: { ...fx },
            refreshIntervalSeconds: MARKET_CLOSED_REFRESH_INTERVAL_SECONDS,
          };
          marketRefreshIntervalSeconds = MARKET_CLOSED_REFRESH_INTERVAL_SECONDS;
          ...
          errors.push(fallbackError.message || "미국주식 지연 시세를 불러오지 못했습니다.");
