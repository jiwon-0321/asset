...
function collectTrackedInstruments(holdings, targets) {
  ...
  holdings.forEach((item) => {
    ...
    const key = `${enriched.market || "unknown"}::${enriched.symbol || enriched.asset}`;
    unique.set(key, {
  ...
  (targets.groups || []).forEach((group) => {
    (group.items || []).forEach((item) => {
      const key = `${item.market || "unknown"}::${item.symbol || item.name}`;
      if (!unique.has(key)) {
        unique.set(key, {
          name: item.name,
          asset: item.name,
...
async function buildLivePriceSnapshot({ rootDir, fetchImpl = fetch }) {
  ...
  if (hasFreshCryptoCache) {
    Object.assign(
      quotes,
      pickCachedQuotes(livePriceCache.crypto.quotes, instruments.filter((item) => item.priceSource === "upbit" && item.symbol))
    );
  } else {
    try {
      ...
      if (hasAllCachedQuotes(livePriceCache.crypto.quotes, instruments.filter((item) => item.priceSource === "upbit" && item.symbol))) {
        Object.assign(
          quotes,
          pickCachedQuotes(livePriceCache.crypto.quotes, instruments.filter((item) => item.priceSource === "upbit" && item.symbol), true)
        );
      }
    }
  }
  ...
  const hasFreshMarketCache =
    isCacheFresh(livePriceCache.market.updatedAt, MARKET_REFRESH_INTERVAL_SECONDS) &&
  ...
  const liveSummary = rebuildSummary(portfolio.summary, liveAssetStatus, portfolio.cashPositions || [], portfolio.realized || []);
  ...
  return {
    ...
    live: {
      ...
      marketRefreshIntervalSeconds: MARKET_REFRESH_INTERVAL_SECONDS,
      status,
      errors,
      warnings,
    },
