...
const MARKET_REFRESH_INTERVAL_SECONDS = 180;
...
async function buildLivePriceSnapshot({ rootDir, fetchImpl = fetch }) {
  ...
  const hasFreshGlobalCryptoCache =
    ...
    hasAllCachedQuotes(livePriceCache.globalCrypto.references, instruments.filter((item) => item.market === "crypto"));
  ...
  if (cryptoInstruments.length) {
    if (hasFreshGlobalCryptoCache) {
      ...
      try {
        ...
        applyKimchiPremiumToQuotes(quotes, globalReferences);
      } catch (error) {
        warnings.push(`kimchi-premium:${error.message}`);
        if (hasAllCachedQuotes(livePriceCache.globalCrypto.references, instruments.filter((item) => item.market === "crypto"))) {
          applyKimchiPremiumToQuotes(quotes, livePriceCache.globalCrypto.references);
        }
      }
    }
