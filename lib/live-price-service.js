...
async function buildMarketSnapshot({
  ...
  const marketOpenNow = isUsRegularMarketOpen(nowDate);
  const tradingDayNow = isUsTradingDay(nowDate);
  const marketRefreshIntervalSeconds = resolveMarketRefreshInterval(nowDate);
  const hasCachedStockQuotes = hasAllCachedQuotes(livePriceCache.market.quotes, usStockInstruments, {
    requireAvailable: true,
  });
  const hasOpenSessionCachedStockQuotes = hasOpenSessionCachedQuotes(livePriceCache.market.quotes, usStockInstruments);
  const hasCachedFxRate = Number.isFinite(Number(livePriceCache.market.fx?.usdkrw)) && Number(livePriceCache.market.fx?.usdkrw) > 0;
  const regularSessionRefreshDue =
    usStockInstruments.length > 0 &&
    marketOpenNow &&
    (forceRefresh || !hasCachedStockQuotes || !isCacheFresh(livePriceCache.market.updatedAt, MARKET_REFRESH_INTERVAL_SECONDS));
  const closedSessionRefreshDue =
    usStockInstruments.length > 0 &&
    !marketOpenNow &&
    (!hasCachedStockQuotes ||
      hasOpenSessionCachedStockQuotes ||
      (tradingDayNow &&
        (forceRefresh || shouldRefreshDaily(livePriceCache.market.updatedAt, US_MARKET_TIME_ZONE, nowDate))));
  const stockRefreshDue = regularSessionRefreshDue || closedSessionRefreshDue;
  const fxRefreshDue = !hasCachedFxRate || shouldRefreshDaily(livePriceCache.market.fx?.updatedAt, FX_REFRESH_TIME_ZONE, nowDate);
  const canServeCachedSnapshot = hasCachedFxRate && (usStockInstruments.length === 0 || hasCachedStockQuotes);
  ...
  if (allowCachedResponse && canServeCachedSnapshot && !forceRefresh) {
    ...
    if (usStockInstruments.length) {
      Object.assign(marketQuotes, pickCachedQuotes(livePriceCache.market.quotes, usStockInstruments, stockRefreshDue));
    }
  ...
  if (useFallbackProvider && stockRefreshDue) {
  ...
  if (stockRefreshDue) {
    ...
    stockResults.forEach((result, index) => {
      ...
      refreshedMarketQuotes[instrument.name] =
        getReusableCachedQuote(livePriceCache.market.quotes, instrument, { markDelayed: marketOpenNow }) ||
        createUnavailableQuote(instrument, "현재가를 불러오지 못했습니다.");
    });

    livePriceCache.market = {
      ...livePriceCache.market,
      updatedAt: nextMarketUpdatedAt,
      quotes: {
  ...
  } else if (hasCachedStockQuotes) {
    Object.assign(marketQuotes, pickCachedQuotes(livePriceCache.market.quotes, usStockInstruments));
  } else {
    usStockInstruments.forEach((instrument) => {
      marketQuotes[instrument.name] = createUnavailableQuote(
        instrument,
        marketOpenNow ? "현재가를 불러오지 못했습니다." : "미국 정규장 시작 후 다시 갱신됩니다."
      );
    });
  }

  if (!stockRefreshDue && fxRefreshDue) {
    livePriceCache.market = {
      ...livePriceCache.market,
      fx: { ...fx },
      indices: createEmptyMajorIndexState(),
