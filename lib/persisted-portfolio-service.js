...
const { normalizeUiPreferences, resolveBoardVariant, resolvePortfolioVariant } = require("./board-config");
...
function withNormalizedUiPreferences(portfolio = null, fallbackVariant = resolveBoardVariant()) {
  const next = structuredClone(portfolio || {});
  const variant = resolvePortfolioVariant(next, fallbackVariant);
  next.metadata = {
    ...(next.metadata || {}),
    boardVariant: variant,
  };
  next.uiPreferences = normalizeUiPreferences(next.uiPreferences, variant);
  return next;
}
...
async function mutatePortfolio(rootDir, fallbackPortfolio, stateKey, mutate, options = {}) {
  ...
  return runSerializedPortfolioMutation(stateKey, async () => {
    const portfolio = withNormalizedUiPreferences(
      await loadPersistedPortfolio(rootDir, fallbackPortfolio, stateKey),
      options?.variant
    );
...
async function getCurrentPortfolio(rootDir = process.cwd(), fallbackPortfolio = null, stateKey = "owner", options = {}) {
  const portfolio = withNormalizedUiPreferences(
    await loadPersistedPortfolio(rootDir, fallbackPortfolio, stateKey),
    options?.variant
  );
...
async function updateUiPreferencesEntry(rootDir = process.cwd(), input = {}, fallbackPortfolio = null, stateKey = "owner", options = {}) {
  return mutatePortfolio(rootDir, fallbackPortfolio, stateKey, async (portfolio) => {
    const variant = resolvePortfolioVariant(portfolio, options?.variant);
    const next = structuredClone(portfolio);
